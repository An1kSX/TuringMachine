...
def mt_code_read(path: str) -> tuple:
    ...
    try:
        ...
        if df.isnull().values.any():
            logs = ('Неправильно заполнена таблица, либо пустые значения в' +
                    ' ячейках. Заполняйте строго по шаблону, прикрепленному к' +
                    ' разделу на контесте'
                    )
            return program, logs
        ...
        if abs(max(df['A'].values.tolist())) > 1 or abs(max(df['ψ'].values.tolist())) > 1:
            logs = ('Входной и выходной алфавиты Машины Тьюринга' +
                    ' могут состоять только из 0 и 1'
                    )
            program = {}

        elif move_alphabet.union({'L', 'R', 'S'}) != {'L', 'R', 'S'}:
            logs = ('Для перемещения по ленте можно использовать только' +
                    ' нижеследующие символы:' +
                    '\nL - налево\nR - направо\nS - остановка'
                    )
            program = {}
        ...
        if 'invalid literal for int() with base 10:' in str(e):
            logs = 'Cпутаны столбцы состояний и значений Машины Тьюринга'
